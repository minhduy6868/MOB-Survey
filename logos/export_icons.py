"""Rasterize the Trovey Horizon-T mark to PWA and Android launcher sizes."""
...
def render(size: int, *, maskable: bool = False, rounded: bool = False) -> Image.Image:
    ...
    # Maskable / adaptive: keep the mark inside the center 66–80% safe zone.
    inset = 0.18 if maskable else 0.06
    sun = _box(size, 168, 96, 176, 176, inset)
    bar = _box(size, 64, 268, 384, 72, inset)
    stem = _box(size, 220, 268, 72, 176, inset)
    draw.ellipse(sun, fill=SUN)
    draw.rounded_rectangle(bar, radius=max(3, (bar[3] - bar[1]) // 2), fill=PAPER)
    draw.rounded_rectangle(stem, radius=max(3, (stem[2] - stem[0]) // 2), fill=PAPER)
    if rounded:
...
def main() -> None:
    ...
    web_sizes = (48, 72, 96, 144, 180, 192, 256, 384, 512)
    for px in web_sizes:
        save_png(render(px), export / f"icon-{px}.png")
        save_png(render(px), ICONS / f"Icon-{px}.png")
    ...
    for folder, px in android_mipmap.items():
        save_png(render(px, rounded=True), ANDROID / folder / "ic_launcher.png")
        save_png(render(px, maskable=True), ANDROID / folder / "ic_launcher_foreground.png")
    save_png(render(512), ANDROID / "play_store_512.png")
    save_png(render(1024), ANDROID / "play_store_1024.png")
